...
cmd({ 
  ...
  try { 
    await conn.sendMessage(from, { text: '📡 Please wait... Checking for SADE updates...' }, { quoted: mek });
    ...
    if (localCommit === originCommit) { 
      await conn.sendMessage(from, { text: '*✅ SADE Bot is already up to date!*' }, { quoted: mek });
    } else { 
      ...
      await conn.sendMessage(from, { text: '*✅ SADE Bot updated successfully!*' }, { quoted: mek });
    }
  } catch (error) { 
    console.error(error); 
